...
if btn:
    if not elem_props:
        status_msg = "Missing elemental properties data"
        st.error("Cannot find or read data/elemental_properties.xlsx")
    elif not models:
        status_msg = "No models loaded"
        st.error("No prediction models found in final_models/ folder")
    else:
        try:
            ...
            for i, key in enumerate(["S", "Sigma", "Kappa", "zT"]):
                ...
                fig.update_layout(
                    ...
                    height=340,                      # ← balanced for 4 plots on screen
